...
<html lang="PT-pt">
  ...
  <body>
    ...
    DriverManager.getConnection(url, utilizador, pass); st =
    conexao.prepareStatement("SELECT * FROM tarefa WHERE id = ?"); st.setInt(1,
    id); rs = st.executeQuery(); if(!rs.next()){ out.println("Erro."); } else{
    %>
    <h1 style="margin: 20px 20px 20px 20px">Tarefas</h1>
    <form action="inserirAtualizacao.jsp" method="post">
      <input type="hidden" name="id" value="<%= rs.getInt("id") %>">
      <div class="col" id="descricao-css">
        <input type="text" class="form-control" aria-label="Descrição"
        name="descricao" value="<%= rs.getString("descricao") %>">
      </div>
      <div class="col" id="categoria-css">
        <input type="text" class="form-control" aria-label="Categoria"
        name="categoria" value="<%= rs.getString("categoria") %>">
      </div>
      <div class="col" id="prioridade-css">
        <input type="text" class="form-control" aria-label="Prioridade"
        name="prioridade" value="<%= rs.getString("prioridade") %>">
      </div>
      <button class="btn btn-outline-primary" type="submit" id="botao-guardar">
        Guardar
      </button>
    </form>
